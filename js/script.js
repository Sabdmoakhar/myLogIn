...
<form id= "theForm" class="position-absolute top-0 end-0 start-0 bottom-0">
               <h3 class="texts">Welcome! Login Here</h3>
...
const userCards = () => {
  ...
  <img src="${loggedInUser.picture}" style="border-radius: 50%" class="card-img-top" alt="">
  <div class="card-body">
    ...
    // If the logged in user is Finance Advisor show all users and admins
    // const usersToDisplay = loggedInUser.role === 'Finance Advisor' ? users : users.filter(u => u.role === 'user');
    let displayUser;
    if (loggedInUser.roleIndicator) {
        //getting all the users and admins except for the logged in Finance Advisor as we have seperate card for it above
        ...
        <div class="grid">
            <div class="grid-item grid-item--width2">
                <div class="card p-3 m-2" >
                    <img id="cardImages" class="card-img-top" src="${user.picture}" style="border-radius: 50%" alt="Clients">
                    <div class="card-body">
          ...
          //selecting all the delete buttons and loop to find the ones that are attached to Finance Advisor cards
          document.querySelectorAll(".myBtn").forEach((button , i)=>{
            //finding the Finance Advisor card and hide the the delete button
            if(displayUser[i].role == 'Finance Advisor'){
...
function cardManagement(){
    let nameSearch = $('#searchName').val().trim()
    const searchedUser = users.find(user => user.first_name.toLowerCase() === nameSearch.toLowerCase())
    if(searchedUser){
        const cardHtml = `
        <div class="grid">
            <div class="grid-item grid-item--width2">
                <div class="card p-3 m-2" >
                    <img id="cardImages" class="card-img-top" src="${searchedUser.picture}" alt="Clients">
                    <div class="card-body">
                        <h5 class="cardText">Name: ${searchedUser.first_name} ${searchedUser.last_name}</h5>
                        <p class="cardText">Email: ${searchedUser.email}</p>
                          <p class="cardText">Role:${searchedUser.role}</p>
                    </div>
                    <button class="btn myBtn" type="button">Close Account</button>
                <div>
            </div>
        </div>`;
        $(myCards).hide().html(cardHtml).fadeIn(400)
    }else{
        $('#searchName').css("background", "#f8d7da").val('No Such Name');
        $('#searchName').animate({marginLeft:"-10px"},100)
